    ...
    <div id='about' className='min-h-screen w-full '>
         <div className="flex px-[2rem] w-full flex-col md:flex-row md:px-[4rem] justify-between items-start z-30 py-[1rem] md:pt-[4rem] ">
          ...
          <div className="flex flex-col gap-3 pt-[4rem]" data-aos="fade-down">
            ...
            <div data-aos="fade-up" className="w-full    h-[37rem] md:mt-2">
            <MonkCanvas />
            </div>
          </div>
       <div className="grid grid-cols-2 md:grid-cols-4 gap-8  mt-10" >
            {skillsSet.map((skill: {name: string, icon: JSX.Element, icon2?: JSX.Element}, index: number) => (
            <div data-aos="flip-right" key={index} className="flex flex-col cursor-pointer items-center hover:bg-transparent hover:border justify-center gap-4 py-4 px-8 rounded-md w-[10rem] h-[10rem] bg-primary">
             <div className="flex items-center gap-2">
             <p className={`${skill.icon2 ? 'block text-gold' : 'hidden'} text-gold text-[4rem]`}>{skill.icon2}</p>
             <p className="text-[4rem] text-gold">{skill.icon}</p>
             </div>
              <p className="text-[1rem] capitalize text-gold">{skill.name}</p>
            </div>
            ))}
       </div>
       </div>
